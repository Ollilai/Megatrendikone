...
function isCompleteResult(data: unknown): data is AnalysisResult {
    ...
    // Check megatrend analysis - all 4 must be present with subtitle and analysis
    if (!result.megatrendAnalysis) return false;
    const requiredTrends = ['teknologia', 'luonto', 'ihmiset', 'valta'] as const;
    for (const trend of requiredTrends) {
        if (!result.megatrendAnalysis[trend]?.subtitle) return false;
        if (!result.megatrendAnalysis[trend]?.analysis) return false;
    }
    ...
    if (!result.socialContract?.role) return false;
...
function AnalyzeContent() {
    ...
    useEffect(() => {
        ...
        // AbortController to prevent duplicate requests (React 18 StrictMode double-mounts)
    ...
    if (error) {
        return (
            <div className="min-h-screen bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 flex items-center justify-center px-4" role="alert" aria-live="assertive">
                <div className="max-w-md text-center">
                    <p className="text-6xl mb-4" aria-hidden="true">😕</p>
                    <h1 className="text-2xl font-bold text-slate-900 mb-4">Hups! Jotain meni pieleen</h1>
                ...
                <div className="relative z-10 py-8 md:py-12 px-4">
                    ...
                    >
                        ...
                        <div className="text-center mb-8">
                            <p className="text-primary-600 text-sm tracking-[0.2em] mb-2">MEGATRENDIPROFIILI 2026</p>
                            <h1 className="text-3xl md:text-4xl font-bold text-slate-900">{result.company.name}</h1>
                            <p className="text-slate-600 mt-2">{result.company.industry}</p>
                        </div>
                        ...
                        >
                            ...
                            <div className="grid md:grid-cols-2 gap-4">
                                {megatrendKeys.map((key) => {
                                    const trend = MEGATRENDS[key];
                                    const analysis = result.megatrendAnalysis[key];
                                    return (
                                        <div key={key} className="bg-white/50 rounded-xl p-5 border border-slate-200 hover:border-slate-300 transition-colors">
                                            <h3 className="font-bold text-slate-900 text-lg mb-1" style={{ color: trend.color }}>
                                                {trend.label}
                                            </h3>
                        ...
                        <motion.div
                            ...
                            className="bg-gradient-to-br from-amber-50 to-amber-100/50 border border-amber-200 rounded-2xl p-6 md:p-8 mb-12"
                        >
                            <h3 className="text-xl font-bold text-amber-600 mb-4">YLLÄTTÄVÄ UHKA</h3>
                            <h4 className="text-xl font-semibold text-slate-900 mb-3">{result.wildCard.title}</h4>
                            <p className="text-slate-700 leading-relaxed">{result.wildCard.description}</p>
                        </motion.div>

                        {/* Social Contract - Yhteiskuntasopimus */}
                        <motion.div
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ delay: 0.45 }}
                            className="bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-200 rounded-2xl p-6 md:p-8 mb-12"
                        >
                            <h3 className="text-xl font-bold text-purple-600 mb-4 flex items-center gap-2">
                                <span>🤝</span> ROOLI UUDESSA YHTEISKUNTASOPIMUKSESSA
                            </h3>
                            <p className="text-slate-700 leading-relaxed">{result.socialContract.role}</p>
                        </motion.div>
                        ...
                        <motion.div
                            ...
                            className="bg-gradient-to-br from-primary-50 to-accent-50 border border-primary-200 rounded-2xl p-6 md:p-8 mb-8"
                        >
                            <h2 className="text-xl font-bold text-slate-900 mb-4">Moi!</h2>
                            <div className="text-slate-700 leading-relaxed space-y-3 mb-6">
                                <p>
